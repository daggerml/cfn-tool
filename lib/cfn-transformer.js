...
(function() {
  var CfnError, CfnModule, CfnTransformer, ResourceTypes, VERSION, YamlTransformer, assert, clone, fn, fs, indexOfClosingCurly, interpolateSub, log, os, parseKeyOpt, parseKeyOpts, path, topLevelResourceProperties, uuid, yaml,
    indexOf = [].indexOf;
  ...
  CfnTransformer = class CfnTransformer extends YamlTransformer {
    constructor({
      ...
      this.defmacro('Ref', 'Ref', (form) => {
        var bind, dfl, getin, ks, ref, refable, segs;
        if (fn.isArray(form)) {
          dfl = form[1];
        }
        ...
        getin = (m, ks) => {
          ...
          fn.assertOk((ret != null) || (dfl != null), `can't resolve: '${ks.join('.')}'`);
      ...
      //=========================================================================#
      this.defmacro('CfnToolVersion', (form) => {
        form = fn.isArray(form) ? form[0] : form;
        assert(fn.compareSemver(form, VERSION), `template requires cfn-tool version compatible with '${form}'`);
        return null;
      });
  ...
  ${form}
      ...
      this.defmacro('File', (form) => {
        ...
        return this.withCache({
          ...
          return fs.readFileSync(form).toString('utf-8');
        });
      });
      ...
      this.defmacro('Md5', (form) => {
        form = fn.isArray(form) ? form[0] : form;
        return fn.md5(form);
      });
      this.defmacro('Md5File', (form) => {
        form = fn.isArray(form) ? form[0] : form;
        return fn.md5Path(form);
      });
    ...
    addNested(x) {
      ...
      return this.nested = x.reduce(function(xs, x) {
        if (indexOf.call(xs, x) >= 0) {
          return xs;
        } else {
          return xs.concat([x]);
        }
    ...
    packageMacro(form, opts) {
      ...
      if (this.opts.dopackage) {
        this.addNested(this.userPath(Path));
        return this.withCache({
    ...
    validate(file) {
      ...
      try {
        ...
        if (this.opts.continue) {
          ...
          return log.error(e.message, {body});
        } else {
          throw e;
        }
      }
    ...
    pushFile(file, f) {
      var old, ret;
      this.addNested(this.userPath(file));
