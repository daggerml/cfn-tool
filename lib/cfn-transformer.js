...
(function() {
  var CfnError, CfnTransformer, ResourceTypes, YamlTransformer, assert, assertArray, assertObject, assoc, conj, crypto, dbg, deepMerge, fileExt, fs, getIn, hashMap, identity, indexOfClosingCurly, interpolateSub, isArray, isBoolean, isDirectory, isObject, isString, log, md5, md5Dir, md5File, md5Path, merge, mergeStrings, notEmpty, os, parseKeyOpt, parseKeyOpts, path, peek, prependLines, readFile, reduceKv, rmCR, spawnSync, split, topLevelResourceProperties, typeOf, yaml,
    indexOf = [].indexOf;
  ...
  rmCR = function(x = '') {
    var j, l, len, len1, lines, ref1, ref2, v, w;
    lines = [];
    ref1 = x.split(/\r/);
  ...
  CfnTransformer = class CfnTransformer extends YamlTransformer {
    constructor({
        ...
        if (isString(form)) {
          ...
          switch (false) {
            ...
            case peek(this.bindstack)[ref] == null:
              return getIn(this.walk(peek(this.bindstack)[ref]), ks);
            default:
              return {
                Ref: form
              };
      ...
      this.defmacro('Shell', (form) => {
        var env, vars;
        [form = '', vars = {}] = isArray(form) ? form : [form];
        env = Object.assign({}, process.env, vars);
        return this.withCache({
          shell: [this.template, form]
        }, () => {
          return (this.execShell(form, {env}) || '').replace(/\n$/, '');
        });
      });
      this.defmacro('Js', (form) => {
        var args, ret, vals, vars;
        [form = '', vars = {}] = isArray(form) ? form : [form];
    ...
    handleShell(cmd, res, raw) {
      ...
      stdout = rmCR((ref1 = res.stdout) != null ? ref1.toString('utf-8') : void 0);
      stderr = rmCR((ref2 = res.stderr) != null ? ref2.toString('utf-8') : void 0);
    ...
    transformTemplateFile(file) {
      var ret, xformer;
      xformer = new this.constructor({basedir: this.basedir, cache: this.cache, opts: this.opts});
