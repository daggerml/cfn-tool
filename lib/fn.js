...
(function() {
  var CfnError, MOCKS, TESTING, WHITELIST, abortOnException, assert, assertArray, assertObject, assertOk, assertString, assoc, compareSemver, conj, crypto, dbg, deepEqual, deepMerge, execShell, execShellArgs, fileExt, fillfmt, fs, getIn, handleShell, hashMap, identity, invertObj, isArray, isBoolean, isCollection, isDirectory, isNull, isNumber, isObject, isRef, isScalar, isString, isUndefined, log, md5, md5Dir, md5File, md5Path, merge, mergeStrings, mockShell, noMockShell, notEmpty, objKeys, objVals, os, partial, path, peek, prependLines, quote, readFile, reduceKv, rmCR, selectKeys, shprintf, spawn, spawnSync, split, splitfmt, sq, testing, tmpdir, tryExecRaw, typeOf, whiteListed,
    indexOf = [].indexOf;
  ...
  quote = function(type, x) {
    if (type.slice(2) === '!' && (x == null)) {
      throw new Error('missing required value');
    }
    if (x) {
      return sq.quote(type.slice(0, 2) === '%S' ? [x] : sq.parse(x));
    }
  };

  splitfmt = function(x) {
    var i, ix, j, l, len, post, pre, re, ref, s, type, xs, ys;
    re = /(%{([^%]+)?(%[AS]!?)([^}]+)?})/g;
    xs = [...x.matchAll(re)].map(function(x) {
      return x.slice(2, 5).map(function(x) {
        return x || '';
      }).concat([x.index, x[0].length]);
    });
    ix = 0;
    ys = [];
    ref = xs.concat([[]]);
    for (j = 0, len = ref.length; j < len; j++) {
      [pre, type, post, i, l] = ref[j];
      if ((s = x.substring(ix, i != null ? i : x.length))) {
        ys.push(s);
      }
      if (type) {
        ys.push({type, pre, post});
      }
      if (i != null) {
        ix = i + l;
      }
  ...
  fillfmt = function({pre, post, type}, y) {
    var word;
    word = quote(type, y);
    if (word) {
  ...
  shprintf = module.exports.shprintf = function(fmt, ...args) {
    var doseg;
    doseg = function(x, i) {
      if (isString(x)) {
        return x;
      } else {
        return fillfmt(x, args.shift());
      }
    };
    return splitfmt(fmt).map(doseg).filter(function(x) {
      return x;
    }).join('').trim();
  };

  compareSemver = module.exports.compareSemver = function(a, b) {
    ...
    return !(b[0] !== a[0] || b[1] < a[1] || (b[1] === a[1] && b[2] < a[2]));
  };
  ...
  rmCR = module.exports.rmCR = function(x = '') {
    var j, len, len1, lines, m, ref, ref1, v, w;
    lines = [];
    ref = x.split(/\r/);
    for (j = 0, len = ref.length; j < len; j++) {
      v = ref[j];
  ...
  mockShell = module.exports.mockShell = function(cmd, res) {
    return MOCKS[cmd] = res;
  };

  noMockShell = module.exports.noMockShell = function(re) {
    return WHITELIST.push(re);
  };

  whiteListed = function(cmd) {
    return WHITELIST.reduce((function(xs, x) {
      return xs || x.test(cmd);
    }), false);
  };

  spawn = function(cmd, ...args) {
    log.spawn(cmd);
    if (TESTING && !whiteListed(cmd)) {
      return MOCKS[cmd] || {
        status: 0,
        stdout: "bogus stdout",
        stderr: "bogus stderr"
